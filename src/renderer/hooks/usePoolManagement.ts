...
export const usePoolManagement = ({ isLaserSabre, poolMaxScore, showToast }: UsePoolManagementProps) => {
  ...
  const nextPoolRound = useCallback((checkedInFencers: Fencer[]) => {
  ...
  const areAllPoolsComplete = useCallback(() => {
    return pools.every(pool =>
      pool.matches.every(match => match.status === MatchStatus.FINISHED)
    );
  }, [pools]);
