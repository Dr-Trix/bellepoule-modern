...
exports.parseSimpleTXTFile = parseSimpleTXTFile;
 ...
 */
function parseSimpleTXTFile(content) {
    const result = {
        success: false,
        fencers: [],
        errors: [],
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (i === 0 && (line.toLowerCase().includes('nom') ||
        ...
        try {
            ...
            result.errors.push(`Ligne ${i + 1}: ${error instanceof Error ? error.message : 'Erreur de parsing'}`);
        }
    }
 ...
 */
function parseTXTLine(line, lineNumber) {
