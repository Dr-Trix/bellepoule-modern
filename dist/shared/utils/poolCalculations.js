...
exports.generatePoolMatchOrder = generatePoolMatchOrder;
exports.calculateFencerPoolStats = calculateFencerPoolStats;
exports.calculatePoolRanking = calculatePoolRanking;
exports.distributeFencersToPoolsSerpentine = distributeFencersToPoolsSerpentine;
exports.validatePool = validatePool;
exports.calculateOptimalPoolCount = calculateOptimalPoolCount;
exports.calculatePoolMatchCount = calculatePoolMatchCount;
exports.formatRatio = formatRatio;
exports.formatIndex = formatIndex;
exports.calculateQuestPoints = calculateQuestPoints;
exports.calculateFencerQuestStats = calculateFencerQuestStats;
exports.calculatePoolRankingQuest = calculatePoolRankingQuest;
exports.calculateOverallRankingQuest = calculateOverallRankingQuest;
exports.calculateOverallRanking = calculateOverallRanking;
 ...
 */
function generatePoolMatchOrder(fencerCount) {
    const orders = {
        3: [
            [1, 2], [2, 3], [1, 3],
        ],
 ...
 */
function generateGenericMatchOrder(fencerCount) {
    const matches = [];
 ...
 */
function calculateFencerPoolStats(fencer, matches) {
    let victories = 0;
    let defeats = 0;
    let touchesScored = 0;
 ...
 */
function calculatePoolRanking(pool) {
    const rankings = [];
    // Calculer les stats pour chaque tireur
    for (const fencer of pool.fencers) {
        if (fencer.status === types_1.FencerStatus.EXCLUDED ||
            fencer.status === types_1.FencerStatus.FORFAIT ||
            fencer.status === types_1.FencerStatus.ABANDONED) {
            continue;
        }
    ...
    rankings.sort((a, b) => {
        ...
        const directMatch = pool.matches.find(m => (m.fencerA?.id === a.fencer.id && m.fencerB?.id === b.fencer.id) ||
            (m.fencerA?.id === b.fencer.id && m.fencerB?.id === a.fencer.id));
        if (directMatch && directMatch.status === types_1.MatchStatus.FINISHED) {
 ...
 */
function distributeFencersToPoolsSerpentine(fencers, poolCount, separation) {
    ...
    for (const fencer of sortedFencers) {
        ...
        const bestPool = findBestPoolForFencer(fencer, pools, poolIndex, poolCount, direction, separation);
 ...
 */
function findBestPoolForFencer(fencer, pools, startIndex, poolCount, direction, separation) {
...
function validatePool(pool) {
    ...
    const incompleteMatches = pool.matches.filter(m => m.status !== types_1.MatchStatus.FINISHED && m.status !== types_1.MatchStatus.CANCELLED);
 ...
 */
function calculateOptimalPoolCount(fencerCount, minPoolSize = 5, maxPoolSize = 8) {
    // Objectif: avoir des poules de taille similaire entre min et max
    for (let poolCount = Math.ceil(fencerCount / maxPoolSize); poolCount <= Math.ceil(fencerCount / minPoolSize); poolCount++) {
 ...
 */
function calculatePoolMatchCount(fencerCount) {
 ...
 */
function calculateQuestPoints(winnerScore, loserScore) {
 ...
 */
function calculateFencerQuestStats(fencer, matches) {
    let questPoints = 0;
 ...
 */
function calculatePoolRankingQuest(pool) {
    const rankings = [];
    for (const fencer of pool.fencers) {
        if (fencer.status === types_1.FencerStatus.EXCLUDED ||
            fencer.status === types_1.FencerStatus.FORFAIT ||
 ...
 */
function calculateOverallRankingQuest(pools) {
    const allRankings = [];
    pools.forEach(pool => {
 ...
 */
function calculateOverallRanking(pools) {
...
//# sourceMappingURL=poolCalculations.js.map